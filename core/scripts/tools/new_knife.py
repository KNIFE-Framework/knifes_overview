...
Špecializovaný generátor pre KNIFE inštancie (napr. K000123).
Volá ho new_item_instance.py po vyhodnotení typu "knifes".

Zodpovednosť:
- Vytvoriť cieľový priečinok pre inštanciu (pod content_dir).
- Prejsť celý strom šablón v core/templates/content/knifes/body (alebo v ceste z configu).
- Pre každý .md súbor:
...
def _safe_name(raw_name: str, fallback: str = "knife_instance") -> str:
...
def _resolve_body_root(config: Dict[str, Any]) -> Path:
    """Zistí koreňový adresár pre body šablóny KNIFE.

    Preferuje sa hodnota v config/knifes/knifes_config.yml, napr.:
      template_body_root: "core/templates/content/knifes/body"
    ...
    return Path("core/templates/content/knifes/body")
...
def _process_markdown_file(
    ...
    """Spracuje jeden .md súbor šablóny KNIFE.
...
def generate(ctx: Dict[str, Any]) -> None:
    """Hlavný vstupný bod pre generovanie KNIFE inštancie.

    Očakávané položky v ctx:
      - config: načítaný YAML config/knifes/knifes_config.yml
      - content_dir: Path – koreň pre zápis KNIFE inštancií (content/docs/sk/knifes)
      - instance_name: napr. "K000123"
      - explicit_id: napr. "K000123" (môže byť rovnaké ako instance_name)
      - cli_title: ľudský title (napr. "Môj prvý KNIFE")
    ...
    debug_print(debug, f"[KNIFE] target_root={target_root}")

    # Riešenie existujúceho priečinka
    target_index = target_root / "index.md"
    if target_index.exists():
        if exists_mode in ("skip", "merge"):
            debug_print(debug, f"[KNIFE] exists → skip/merge: {target_index}")
            print(
                f"KNIFE instance already exists at: {target_root} (exists={exists_mode})"
            )
            return
        if exists_mode == "error":
            raise SystemExit(
                f"KNIFE instance already exists at: {target_root} (exists=error)"
            )
    ...
    # Zistíme koreň šablóny pre KNIFE
    body_root = _resolve_body_root(config)
    if not body_root.exists() or not body_root.is_dir():
        raise FileNotFoundError(
            f"KNIFE template body root not found or not a directory: {body_root}"
        )

    debug_print(debug, f"[KNIFE] Using body_root={body_root}")
    ...
    for src_path in body_root.rglob("*"):
        ...
        if src_path.suffix.lower() != ".md":
            ...
            debug_print(debug, f"[KNIFE] copied asset: {rel_path}")
            continue

        # Markdown – spracujeme ako šablónu s FM
        debug_print(debug, f"[KNIFE] rendering markdown: {rel_path}")
    ...
    print(
        f"KNIFE instance generated at: {target_root} "
