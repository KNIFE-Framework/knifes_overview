...
Špecializovaný generátor pre know_myself inštancie (napr. K000123).
Volá ho new_item_instance.py po vyhodnotení typu "know_myselfs".

Zodpovednosť:
- Vytvoriť cieľový priečinok pre inštanciu (pod content_dir).
- Prejsť celý strom šablón v core/templates/content/know_myself/body (alebo v ceste z configu).
- Pre každý .md súbor:
  - vygenerovať Front Matter na základe FM-Core + configu + ID + title
  - nahradiť placeholdery z FM-Core v tele šablóny
  - podľa potreby vložiť header template (ak je definovaný)
...
def _safe_name(raw_name: str, fallback: str = "know_myself_instance") -> str:
...
def _resolve_body_root(config: Dict[str, Any]) -> Path:
    """Zistí koreňový adresár pre body šablóny know_myself.

    Preferuje sa hodnota v config/know_myself/know_myself_config.yml, napr.:
      template_body_root: "core/templates/content/know_myself/body"
    ...
    return Path("core/templates/content/know_myself/body")


def _build_combined_title(
    explicit_id: Optional[str],
    cli_title: Optional[str],
    instance_name: str,
) -> Optional[str]:
    """Vytvorí kombinovaný title podľa konvencie: ID – Title"""
...
def _process_markdown_file(
    ...
    """Spracuje jeden .md súbor šablóny know_myself.
    ...
    # Upravíme title tak, aby obsahoval ID + title
    ...
    processed_header = ""
    if inject_header and template_header_path:
        try:
...
def generate(ctx: Dict[str, Any]) -> None:
    """Hlavný vstupný bod pre generovanie know_myself inštancie.

    Očakávané položky v ctx:
      - config: načítaný YAML config/know_myself/know_myself_config.yml
      - content_dir: Path – koreň pre zápis know_myself inštancií (content/docs/sk/know_myself)
      - instance_name: napr. "K000123"
      - explicit_id: napr. "K000123" (môže byť rovnaké ako instance_name)
      - cli_title: ľudský title (napr. "Môj prvý know_myself")
    ...
    template_header_path: Optional[str] = config.get("template_header_path")
    ...
    # Názvy a cesty
    # - explicit_id slúži primárne ako ID vo FM
    # - raw_name (ak je) môže obsahovať hierarchiu priečinkov, napr. "CHAP01/CHAP01.01/KMy000101"
    # - ak raw_name nie je, použijeme jednoúrovňový priečinok podľa ID/instance_name
    if raw_name:
        raw_path = Path(raw_name)
        safe_parts = [_safe_name(part, fallback="segment") for part in raw_path.parts]
        folder_rel = Path(*safe_parts)
    else:
        base_name = explicit_id or instance_name or "know_myself_instance"
        folder_rel = Path(_safe_name(base_name, fallback="know_myself_instance"))

    target_root = content_dir / folder_rel

    debug_print(
        debug,
        f"[know_myself] instance_name={instance_name}, explicit_id={explicit_id}, raw_name={raw_name}",
    )
    debug_print(debug, f"[know_myself] target_root={target_root}")

    # Riešenie existujúceho priečinka
    target_index = target_root / "index.md"
    if target_index.exists():
        if exists_mode in ("skip", "merge"):
            debug_print(debug, f"[know_myself] exists → skip/merge: {target_index}")
            print(
                f"know_myself instance already exists at: {target_root} (exists={exists_mode})"
            )
            return
        if exists_mode == "error":
            raise SystemExit(
                f"know_myself instance already exists at: {target_root} (exists=error)"
            )
    ...
    # Zistíme koreň šablóny pre know_myself
    body_root = _resolve_body_root(config)
    if not body_root.exists() or not body_root.is_dir():
        raise FileNotFoundError(
            f"know_myself template body root not found or not a directory: {body_root}"
        )

    debug_print(debug, f"[know_myself] Using body_root={body_root}")
    ...
    for src_path in body_root.rglob("*"):
        ...
        if src_path.suffix.lower() != ".md":
            ...
            debug_print(debug, f"[know_myself] copied asset: {rel_path}")
            continue

        # Markdown – spracujeme ako šablónu s FM
        debug_print(debug, f"[know_myself] rendering markdown: {rel_path}")
    ...
    print(
        f"know_myself instance generated at: {target_root} "
