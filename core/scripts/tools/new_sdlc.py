...
Špecializovaný generátor pre SDLC inštancie (napr. 2025_ST_001).
...
def _resolve_body_root(config: Dict[str, Any]) -> Path:
    ...
    Preferuje sa hodnota v config/SDLC/sSDLC_config.yml, napr.:
...
def _process_markdown_file(
    ...
    sdlc_id_for_comment = explicit_id or instance_name
    if not is_root and sdlc_id_for_comment:
        parts.append(f"<!-- SDLC_INSTANCE_ID: {sdlc_id_for_comment} -->\n\n")
...
def generate(ctx: Dict[str, Any]) -> None:
    """
    Hlavný vstupný bod pre generovanie SDLF inštancie.
    ...
    Očakávané položky v ctx:
      ...
      - instance_name: napr. "SDLC-2025_ST_001"
      - explicit_id: napr. "SDLC-2025_ST_001" (môže byť rovnaké ako instance_name)
      - cli_title: ľudský title (napr. "Môj prvý SDLC")
    ...
    # Názvy a cesty
    base_name = explicit_id or raw_name or instance_name or "sdlc_instance"
    folder_name = _safe_name(base_name, fallback="sdlc_instance")
    target_root = content_dir / folder_name

    debug_print(debug, f"[SDLC] instance_name={instance_name}, explicit_id={explicit_id}")
    debug_print(debug, f"[SDLC] target_root={target_root}")

    # Riešenie existujúceho priečinka
    target_index = target_root / "index.md"
    if target_index.exists():
        if exists_mode in ("skip", "merge"):
            debug_print(debug, f"[SDLC] exists → skip/merge: {target_index}")
            print(
                f"SDLC instance already exists at: {target_root} (exists={exists_mode})"
            )
            return
        if exists_mode == "error":
            raise SystemExit(
                f"SDLC instance already exists at: {target_root} (exists=error)"
            )
    ...
    # Zistíme koreň šablóny pre SDLC
    body_root = _resolve_body_root(config)
    if not body_root.exists() or not body_root.is_dir():
        raise FileNotFoundError(
            f"SDLC template body root not found or not a directory: {body_root}"
        )

    debug_print(debug, f"[SDLC] Using body_root={body_root}")
    ...
    for src_path in body_root.rglob("*"):
        ...
        if src_path.suffix.lower() != ".md":
            ...
            debug_print(debug, f"[SDLC] copied asset: {rel_path}")
            continue

        # Markdown – spracujeme ako šablónu s FM
        debug_print(debug, f"[SDLC] rendering markdown: {rel_path}")
    ...
    print(
        f"SDLC instance generated at: {target_root} "
