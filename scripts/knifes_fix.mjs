...
async function runReportId(opt) {
  ...
  if (!target) { console.error('❌ --report-id requires an ID value, e.g., K059'); process.exit(2); }
...
async function runReportTree(opt) {
  ...
  if (!target) { console.error('❌ --report-tree requires an ID value, e.g., K083'); process.exit(2); }
  if (!(await pathExists(opt.docsRoot))) { console.error(`❌ Docs root not found: ${opt.docsRoot}`); process.exit(2); }

  // find folder(s) like docsRoot/K083-*
